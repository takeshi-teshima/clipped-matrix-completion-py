...
class DTrCMCRecommender(Recommender):
    def factorize(self, R):
        R = R.copy()
        self.pred = clipping_aware_matrix_completion(
            R, self.censoring_threshold, self.loss_scaling_factor, self.lambda1,
            self.lambda2, self.max_iter, self.eta_t, self.decay_rate,
